...
if len(found) > 0:
    ...
    driver.find_element(By.CLASS_NAME, "button-buy").click()
    sleep(1)
    driver.find_element(By.CLASS_NAME, "confirm").click()
    ...
    while not is_form_loaded:
        try:
            form = driver.find_element(By.CLASS_NAME, "login100-form")
            is_form_loaded = True
        except NoSuchElementException:
            print("Puessss no esta el formulario...")

    email = form.find_element(By.NAME, "jform[email]")
    password = form.find_element(By.NAME, "jform[password]")
